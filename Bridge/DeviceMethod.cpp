...
bridge::DeviceMethod::InvokeMethod(ajn::Message& msg, std::vector<ajn::MsgArg>& outArgs)
{
  ...
  std::shared_ptr<adapter::IoRequest> request;

  outArgs.clear();

  for (auto const &p : m_adapterMethod->GetInputArguments())
  {
    adapter::Value param = p;

    ajn::MsgArg const* inArg = msg->GetArg(i++);
    if (inArg == nullptr)
      return 1;

    // TODO: this seems odd. The GetValue() populates the param argument with the
    ...
    status = AllJoynHelper::GetValue(param, *inArg);
    if (status != ER_OK)
      return 1;
  }
...
QStatus
bridge::DeviceMethod::Initialize(adapter::Method const&)
{
...
QStatus
bridge::DeviceMethod::BuildSignature(adapter::Value::Vector const&, std::string&, std::string&)
{
  DSBLOG_NOT_IMPLEMENTED();
  return ER_NOT_IMPLEMENTED;
}
